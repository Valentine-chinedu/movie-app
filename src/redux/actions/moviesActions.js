		...
		try {
			if (movieId) {
				...
				const {
					...
					`${process.env.REACT_APP_URI}movie/${otherListName}?api_key=${process.env.REACT_APP_API_KEY}&page=${page}`
				);
				dispatch({ type: FETCH_MOVIE_LIST_SUCCESS, payload: results });
...
const getMovieDetails = (id) => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(
			`${process.env.REACT_APP_URI}movie/${id}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`
		);
...
const getMovieVideo = (id) => async (dispatch) => {
	try {
		...
		const {
			...
			`${process.env.REACT_APP_URI}movie/${id}/videos?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`
		);
